...
void main(void)
{
	...
	vec3 position = vec3(in_Position.x,0.0,in_Position.z)*20.0 - 10.0; //u_GridSize + u_GridOffset;
	
	/* Read vertex y-value (height) from heightmap */
	float height = texture(u_HeightMap,in_Position.xz).x; //vec2(1.0-in_Position.xxxxxxx,1.0-in_Position.y)
	/* Scale and bias */
