...
GPUClothSimulation::GPUClothSimulation(GLuint * w,GLuint *h)
{
    ...
    **/
    ...
    GLuint verletShader = loadShaders("shaders/mass_spring_verlet.vert","shaders/mass_spring_verlet.frag");
...
**/
unsigned long  GPUClothSimulation::getSpringState(GLuint x,GLuint y){
        ...
        for(i = 0; i < 12; i++){
            ...
            if( yOffset < GPU_CLOTH_DIM  && yOffset >= 0 && xOffset < GPU_CLOTH_DIM  && xOffset >= 0 ){
                unsigned long temp = (unsigned long)pow(2,state);
                springState |= temp;
            }
        }
           return springState;
